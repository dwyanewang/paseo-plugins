...
function setup(
  ...
  const engine = new NotificationEngine({
    clock,
    sender: { send: async (message) => (sent.push(message.summary), bodies.push(message.body), "message-id") },
    inspect: async () => inspection,
    ...(isUserPresent ? { isUserPresent } : {}),
    log: (message) => logs.push(message),
  });
  return { clock, sent, bodies, logs, engine, setInspection: (next: RuntimeInspection) => (inspection = next) };
}
...
describe("NotificationEngine", () => {
  ...
  it("用户正在使用 Paseo 时跳过，离开后照常推送", async () => {
    ...
    expect(sent).toEqual([]);
    expect(logs).toContain("用户正在使用 Paseo，跳过邮件通知");
