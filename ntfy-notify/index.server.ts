...
import { hasToken, readConfig, readCredentials, writeConfig } from "./server/config";
...
import { readNtfyConfig, saveNtfyConfig, sendTestNotification } from "./shared/ntfy";
...
export default function contribute(server: PluginServerContext) {
  const log = (message: string, details?: Record<string, unknown>) => {
    // Never include the access token in plugin logs.
    console.log(`[ntfy-notify] ${message}`, details ?? "");
  };
  const sender = createNtfySender(server.secrets, { log });
  ...
  void readCredentials(server.secrets)
    .then((credentials) => {
      ...
      log("未配置 ntfy 服务器，在 设置 → 插件 → ntfy-notify 里填写");
      return false;
    })
    .catch((error: unknown) => log("读取 ntfy 配置失败", { error: error instanceof Error ? error.message : String(error) }));

  server.handle(readNtfyConfig, async () => ({
    config: await readConfig(server.secrets),
    hasToken: await hasToken(server.secrets),
  }));
  server.handle(saveNtfyConfig, async ({ token, clearToken, ...config }) => {
    await writeConfig(server.secrets, config, token.trim(), clearToken);
    return { status: "saved" as const };
  });
  server.handle(sendTestNotification, async () => {
