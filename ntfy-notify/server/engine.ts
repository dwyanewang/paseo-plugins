...
import { firstErrorLine, formatBody, formatMerged, formatPriority, formatSubject } from "./format";
...
export class NotificationEngine {
  ...
  onTurnEnded(event: TurnEndedEvent): void {
    ...
    // Claude Code reports quota exhaustion as a normal completion whose reply is the notice.
    if (outcome.kind === "completed" && turn?.limitNotice) {
      ...
      this.enqueue({
        ...
        errorFirstLine: firstErrorLine(turn.reply ?? "达到用量限额"),
        error: turn.reply ?? undefined,
  ...
  async flush(): Promise<void> {
    ...
    if (await this.userPresent()) {
      this.deps.log?.("用户正在使用 Paseo，跳过 ntfy 通知", { count: records.length });
      return;
    }
    try {
      await this.deps.sender.send({
        title: formatSubject(records),
        summary: formatMerged(records),
        body: formatBody(records),
        card: buildCard(records),
        priority: formatPriority(records),
      });
    } catch (error) {
      this.deps.log?.("ntfy 通知发送失败", {
        error: error instanceof Error ? error.message : String(error),
      });
    }
  }
