  ...
  return (
    <View style={styles.screen}>
      ...
      <View style={styles.rowWrap}>
        ...
        <Button styles={styles} theme={theme} label="Reset Todo data" icon="Trash2" variant="danger" disabled={recovery.saving} onPress={() => setConfirm(true)} />
      </View>
      {recovery.saveError ? <Text style={styles.danger}>{recovery.saveError}</Text> : null}
      <ConfirmBox
        theme={theme}
        request={confirm ? { title: "Reset Todo data?", message: TEXT.resetWarning, label: "Reset now", requireDouble: true, icon: "Trash2" } : null}
